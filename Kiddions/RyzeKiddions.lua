...
badSp:add_action("Añadir malos jugadores", function() stats.set_int("MPPLY_BADSPORT_MESSAGE", -1) stats.set_int("MPPLY_BECAME_BADSPORT_NUM", -1) stats.set_float("MPPLY_OVERALL_BADSPORT", 60000) stats.set_bool("MPPLY_CHAR_IS_BADSPORT", true) globals.set_int(1575015, 0) globals.set_int(1574589, 1) sleep(0.2) globals.set_int(1574589, 0) end)
...
OnlMenu:add_action("Remover tiempo de espera de VIP/MC", function() stats.set_int("MPPLY_VIPGAMEPLAYDISABLEDTIMER", 0) end)
...
OnlMenu:add_action("Desbloquear Garage 50", function()
...
spoof:add_int_range("Valor", 1, 1, 2147483647, function()
end, function(value)
	if skidy_1 == 1 then
		rank = value * steps
...
spoof:add_float_range("Valor", 0.01, 0.01, 2147483647.00, function()
end, function(value)
	if skidy_2 == 1 then
		kd = value * steps2
	end
...
Text("----")
spoof:add_toggle("Activar/Desactivar", function()
	return boolsr
end, function()
	boolsr = not boolsr
	sad(boolsr)
...
Text("--")
local cus = spoof:add_submenu("Opciones")
cus:add_toggle("Adaptar RP al Rango", function()
	return boolrp 
end, function()
	boolrp = not boolrp
end)
cus:add_toggle("Adaptar KD a Kills y Muertes", function()
	return boolkd
end, function()
	boolkd = not boolkd
end)
